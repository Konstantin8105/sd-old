...
func delta(a, b forceCase2d) d float64{
	for i := range a.gravityForces{
		d  = math.Max(d, max.Abs(a.gravityForces[i] - b.gravityForces[i])/math.Max(math.Abs(a.gravityForces[i]),math.Abs(b.gravityForces[i])))
	}
	for i := range a.nodeForces{
		d  = math.Max(d, max.Abs(a.nodeForces[i] - b.nodeForces[i])/math.Max(math.Abs(a.nodeForces[i]),math.Abs(b.nodeForces[i])))
	}
	return
}
